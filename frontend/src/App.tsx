...
function formatDateRange(filters: FilterState): string {
  ...
  if (start_date) {
    const start = new Date(start_date);
    return `From ${start.toLocaleDateString("en-US", { month: "short", year: "numeric" })}`;
  }
  if (end_date) {
    const end = new Date(end_date);
...
function getActiveFiltersSummary(filters: FilterState): string[] {
  ...
  if (filters.regions.length > 0) {
    summary.push(filters.regions.length === 1 ? filters.regions[0] : `${filters.regions.length} Regions`);
  }
  if (filters.segments.length > 0) {
    summary.push(filters.segments.length === 1 ? filters.segments[0] : `${filters.segments.length} Segments`);
  }
  if (filters.categories.length > 0) {
    summary.push(filters.categories.length === 1 ? filters.categories[0] : `${filters.categories.length} Categories`);
  }
  return summary;
}

function hasActiveFilters(filters: FilterState): boolean {
  return !!(filters.start_date || filters.end_date || filters.regions.length > 0 || filters.segments.length > 0 || filters.categories.length > 0);
}
    ...
    <div className="dashboard">
          ...
          <div className="context-left">
            ...
            {filterSummary.length > 0 && (
              <>
                ...
                <span className="context-filters">{filterSummary.join(", ")}</span>
              </>
            )}
          </div>
      ...
      {!filters.start_date || !filters.end_date ? (
        <section className="no-data-message">
          ...
          <p>Choose a time period using the Quick Select buttons or Custom Date Range above to see your analytics.</p>
